// **todos.js** is the biggest file that contains most of logic because application is tiny. In bigger projects is better to split up your application parts. ``Todos`` class extends ``MK.Array`` class which means "collection".
"use strict";
this.Todos = Class({
	'extends': MK.Array,
	Model: Todo,
	// [itemRenderer](http://finom.github.io/matreshka/docs/Matreshka.Array.html#itemRenderer) method renders element for each added item. At this case it takes template in element that has ``id="todo_item_template"``
	...
	bindings: function() {
		return this
			// We're binding main alament which has "todoapp" ID. After that we'r binding another elements (main, footer etc).
	...
	events: function() {
		return this
			...
			// Code abowe shows that ``"allCompleted"`` property is associated with ``#toggle-all`` element. When it's value is changed we're setting same ``"completed"`` peoperty value for all items we have. ``"silent"`` flag says that ``"change:completed"`` event has not to be fired.
