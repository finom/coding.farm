...
MK = Matreshka = Class({
	 ...
	 *   alert( 'bar' );
	 * }, true );
	 * @example <caption><code>change:KEY</code> event</caption>
	 * this.on( 'change:x', function( evt ) {
	 * 	alert( evt.value );
	 * });
	 * this.x = 1;
	 * @example <caption>DOM events for bound elements (<code>DOM_EVENT::KEY</code>) (since v0.1.0)</caption>
	 ...
	 * });
	 * @example <caption>Bubbling events (<code>KEY@EVENT_NAME</code>) (since v0.2.0)</caption>
	 ...
	 * this.x.a = 1;
	 */
	on_OLD: function ( names, callback, triggerOnInit, context, xtra ) {
	 ...
	 */
	off_OLD: function (names, callback, context) {
	 ...
	 * @param {(Node[]|NodeList|Node|jQuery|balalaika|string)} el - An element (DOM Node or DOM NodeList or array of nodes or balalaika array or jQuery instance or css selector...) that has to be binded to given key(s). Pay attention that element is required, otherwise error will be thrown.
	 ...
	 * @summary Returns collection of elements (wrapped with jQuery or balalaika) that bound to given property (or properties)
	 * @desc After you bound elements to a property you can extract them by using this method.
	 * @param {string} [key] - For which key or space-delimited keys we want to extract elements. If key is undefined or null the method returns elements bound to <code>this</code>.
	 ...
	 * @summary Returns collection of elements (wrapped with jQuery or balalaika) that bound to given property (or properties). Does the same as {@link Matreshka#boundAll}
	 */
	$bound: function( key ) {
 ...
 * this.x = new MK;
 * this.x.a = 1;
 * @example <caption>Bubbling events (<code>@EVENT_NAME</code>) for <code>MK.Object</code> members (since v0.2.0)</caption>
 * this.on( '@change:a', function() {
 * 	alert( '"a" is changed' );
 * });
 * this.jset( 'x', new MK );
 * this.x.a = 1;
 * @example <caption>Bubbling events (<code>@EVENT_NAME</code>) for <code>MK.Array</code> members (since v0.2.0)</caption>
 * this.on( '@change:a', function() {
 * 	alert( '"a" is changed' );
 * });
 * this.push( new MK );
...
(function (root, factory) {
	...
	return MK.Object = MK.Class({
		 ...
		 * @since 0.2
		 * @summary Works same as {@link Matreshha#on} and allows to attach event handlers for any existing and furite items
		 * @example
