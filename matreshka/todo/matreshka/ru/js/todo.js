// **todo.js** содержит ``Todo`` класс, который используется, как модель (``Model``) для главного класса (``Todos``). Класс ``Todo`` наследуется от класса [MK.Object](http://finom.github.io/matreshka/docs/Matreshka.Object.html).
"use strict";
this.Todo = Class({
	'extends': MK.Object,
		...
		this
			...
			.on( 'render', function( evt ) {
				this
					...
					// * Свойство ``"destroy"`` привязывается к элементу с классом ``destroy``, который не имеет стандартного привязчика. Это значит, что элемент просто ассоциируется со свойством, не синхронизируясь с ним.
					.bindElement({
					...
					// Когда срабатывает события, мы меняем "режим" экземпляра на редактирование, присваивая свойству ``"editing"`` значение ``true``. Это действие добавляет класс ``"edit"`` главному элементу (см. привязки выше).
